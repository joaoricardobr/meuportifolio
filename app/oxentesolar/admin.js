...
async function uploadFile(file, pathPrefix) {
    ...
    const randomString = Math.random().toString(36).substring(2, 12); // Shorter random string
    ...
    if (!urlData?.publicUrl) {
        console.error("Failed to get public URL after upload for path:", filePath);
        throw new Error("Falha ao obter URL pública do arquivo após upload.");
    }
...
async function deleteFile(fileUrl) {
    ...
    try {
        ...
        // Pathname often starts with /storage/v1/object/public/bucket-name/
        ...
        if (error) { console.error("Storage Delete Error:", error); /* Log only, don't block further actions */ }
...
function updateUI(user) {
    ...
    if (user) { // Logged In
        ...
        // Load data only if the admin panel wasn't already visible
...
async function loadAdminData() {
     ...
     try {
         ...
         ]).catch(err => { throw new Error(`Network or Supabase error during fetch: ${err.message}`)});
         ...
         if (settings) {
             ...
             console.warn("No initial settings found in DB. Forms will be empty.");
         }
         ...
         // Clear list placeholders on error too
...
function renderAdminList(type, items) {
    ...
    // Sort items if order_index exists, otherwise keep DB order
    ...
    sortedItems.forEach(item => {
        ...
        div.className = 'list-item group relative'; // Add relative for absolute positioning
        div.dataset.id = item.id;
        div.dataset.type = type; // Store type for editing
        ...
        let itemContent = `<div class="item-content pr-20">`; // Add padding for controls
        itemContent += `<h5 class="">${item.title || item.name || `Item [${item.id.substring(0, 6)}]`}</h5>`; // Title/Name
    ...
    listContainer.querySelectorAll('.edit-item-btn').forEach(btn => btn.onclick = handleEditItem); // Still needs implementation
}
...
async function handleEditItem(event) {
     ...
     const tableName = type === 'blog' ? 'blog_posts' : type;
     const statusElement = button.closest('.list-item')?.querySelector('.status-message');

     alert(`Editar Item (Implementação Pendente):\n\nTipo: ${type}\nID: ${id}\nTabela: ${tableName}\n\nPróximos Passos:\n1. Buscar dados do item no Supabase.\n2. Preencher um formulário de edição (talvez um modal).\n3. Implementar a função de salvar as alterações (supabase.update).`);

     // --- Placeholder for actual implementation ---
     // if (!supabase) return;
     // showStatus(statusElement, 'Carregando para editar...', 'loading');
     // try {
     //     const { data: itemData, error } = await supabase.from(tableName).select('*').eq('id', id).single();
     //     if (error) throw error;
     //     if (!itemData) throw new Error("Item não encontrado.");
     //
     //     // TODO: Populate an edit form (create one or reuse 'add' form fields in a modal)
     //     console.log("Data to edit:", itemData);
     //     // Example: populateAddFormWithEditData(type, itemData); // Hypothetical function
     //     showStatus(statusElement, 'Pronto para editar (funcionalidade pendente).', 'success');
     //
     // } catch (error) {
     //     console.error(`Error fetching item ${id} for edit:`, error);
     //     showStatus(statusElement, `Erro ao carregar para edição: ${error.message}`, 'error');
     // }
     // --- End Placeholder ---
}
...
async function handleDeleteItem(event) {
    ...
    const id = button.dataset.id;
    const imageUrl = button.dataset.imgUrl; // Retrieve image URL associated with the item
    const listItemElement = button.closest('.list-item');
    const statusElement = listItemElement?.querySelector('.status-message');

    if (!supabase || !id || !tableName || !listItemElement) {
        console.error("Delete prerequisite missing"); return;
    }
    if (!confirm(`Tem certeza que deseja excluir este item da tabela "${tableName}"? A imagem associada (se houver) também será excluída do armazenamento.`)) {
        return;
    }
    ...
    try {
        ...
        // Attempt to delete image from storage only AFTER successful DB deletion
        ...
        showStatus(statusElement, `Erro ao excluir: ${error.message}`, 'error');
        setLoading(button, false); // Re-enable button on error
    }
    // No finally for setLoading here, as the button is removed on success
}
    ...
    try {
        ...
        let imageInputName = '';
         ...
         for (let [key, value] of formData.entries()) {
             ...
             } else if (key === 'features_text') {
                 itemData['features'] = value.split('\n').map(f => f.trim()).filter(Boolean);
             } else if (key === 'published_at_input') {
                 itemData['published_at'] = value ? new Date(value).toISOString() : new Date().toISOString();
             } else if (!key.endsWith('_file') && !key.endsWith('_input')) { // Exclude _file and specially named _input
        ...
        // Basic slug generation if adding blog post and slug is empty
        if(tableName === 'blog_posts' && !itemData.slug && itemData.title) {
            itemData.slug = itemData.title.toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '');
            console.log("Generated slug:", itemData.slug);
        }
         ...
         if (error) {
             ...
             if (error.code === '23502') { // Not-null violation
                 ...
                 throw new Error(`Campo obrigatório "${match ? match[1] : 'desconhecido'}" não pode ser vazio.`);
             }
             throw error;
         }
...
async function handleSaveSettings(event) {
     ...
     if (!supabase) return;
     const form = event.target;
     const tableName = form.dataset.table; // e.g., 'website_settings'
     const recordId = parseInt(form.dataset.id, 10); // e.g., 1
     const statusElementId = form.id.replace('admin-form-', 'status-'); // e.g., status-settings
     const saveButton = form.querySelector('button[type="submit"]');
     if (!tableName || isNaN(recordId)) { alert("Erro: Formulário sem config. de tabela/ID."); return; }
     ...
     try {
         ...
         let currentImageUrl = form.querySelector(`input[type="hidden"][name$="_url"]`)?.value || ''; // Get current URL from hidden field
         ...
         for (let [key, value] of formData.entries()) {
             if (key.endsWith('_file') && value instanceof File && value.size > 0) {
                 ...
                 dbImageColumn = key.replace('_file', '_url'); // e.g., site_logo_url
                  ...
                  // Re-fetch current URL specifically for the field being uploaded
         ...
         if (fileToUpload && dbImageColumn) {
              ...
              const newImageUrl = await uploadFile(fileToUpload, dbImageColumn.replace('_url','').replace('_image','')); // Use simpler folder name
              updates[dbImageColumn] = newImageUrl; // Add new URL to the updates object
              // Update hidden input immediately for next potential save
         ...
         // Update Database only if there are text changes or a new image was uploaded
         if (Object.keys(updates).length > 0) {
             ...
             console.log("Nenhuma alteração detectada para salvar no banco de dados.");
         }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (!supabase) { // Early exit if Supabase failed
        authContainer.innerHTML = '<p class="text-red-600 text-center font-bold">Erro: Supabase não está configurado corretamente. Verifique URL/Chave em config.js.</p>';
        return;
    }
    ...
    supabase.auth.getSession().then(({ data: { session } }) => {
        ...
        updateUI(null); // Assume logged out on error
    });
    ...
    // Settings Forms (Use generic handler)
