...
async function loadAdminData() {
     // ... (same as before - fetches data and populates forms/lists) ...
     if (!supabase) return;
     console.log("Loading admin data...");
     const statusEl = document.querySelector('#admin-content h3 + p');
...
function renderAdminList(type, items) {
    // ... (same as before - dynamically creates list items with Edit/Delete) ...
...
async function handleDeleteItem(event) {
    ...
    try {
         ...
         if (imageUrl && imageUrl !== 'null' && imageUrl !== '') await deleteFile(imageUrl);
    ...
    // ... (same as before - handles adding new items) ...
     ...
     try {
         ...
         let imageInputName = '';
          ...
          for (let [key, value] of formData.entries()) {
              if (key.endsWith('_file') && value instanceof File && value.size > 0) {
                  ...
                  itemData['published_at'] = value ? new Date(value).toISOString() : null;
         ...
         if(tableName === 'blog_posts' && !itemData.slug && itemData.title) {
             ...
             if(!itemData.slug) itemData.slug = `post-${Date.now()}`;
         }

          const { data, error } = await supabase.from(tableName).insert([itemData]).select();
          if (error) {
              if (error.code === '23505' && error.message.includes('slug')) { throw new Error('O "Slug (URL amigável)" já existe.'); }
              if (error.code === '23502') { const match = error.message.match(/null value in column "(\w+)"/); throw new Error(`Campo obrigatório "${match ? match[1] : '?'}" não pode ser vazio.`); }
              throw error;
          }
...
async function handleSaveSettings(event) {
    // ... (same as before - handles saving settings/about) ...
      event.preventDefault();
      if (!supabase) return;
      const form = event.target;
      ...
      try {
          ...
          if (fileToUpload && dbImageColumn) {
               ...
               // Use a consistent prefix like 'settings' for these uploads
               const folderPrefix = dbImageColumn.includes('logo') ? 'logo' : (dbImageColumn.includes('hero') ? 'hero' : 'about');
               newImageUrl = await uploadFile(fileToUpload, folderPrefix);
          ...
          if (Object.keys(updates).length > 0 || newImageUrl) {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (!supabase) {
        authContainer.innerHTML = '<p class="text-red-600 text-center font-bold">Erro: Supabase não configurado. Verifique config.js.</p>';
        return;
    }
